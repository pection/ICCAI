...
class CLEVRAnswerTable:
    def __init__(self, dsets=None):
        self.vocab = copy.deepcopy(CLEVR_VOCAB)
        self._id2ans_map = {
            self.vocab["answer_token_to_idx"][ans]: ans
            for ans in self.vocab["answer_token_to_idx"]
        }
        self._ans2id_map = self.vocab["answer_token_to_idx"]
    ...
    def used(self, ans):
        return ans in self.vocab["answer_token_to_idx"]
...
def load_lxmert_qa(path, model, label2ans, use_clevr_answer_table=False):
    """
    ...
    :param label2ans: The label2ans dict of fine-tuned QA datasets, like
        {0: 'cat', 1: 'dog', ...}
    :param use_clevr_answer_table: If true, we will use the answer-to-idx
                                   mapping used by CLEVR, otherwise we use
                                   the mapping used by pretrained LXMERT.
                                   You must set this to true if you're trying
                                   to load a model pretrained on CLEVR (or
                                   CLEVR-HO)
    :return:
    """
    ...
    # If we're loading a model pretrained on CLEVR, then we've got a different
    # pre-train time vocab/AnswerTable to load; adjust accordingly
    answer_table = AnswerTable()
    if use_clevr_answer_table:
        answer_table = CLEVRAnswerTable()
